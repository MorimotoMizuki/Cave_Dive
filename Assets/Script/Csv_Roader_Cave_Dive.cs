...
public class Csv_Roader_Cave_Dive : MonoBehaviour
{
    //岩マップ辞書 : 各ステージの名前(例：stage1)をキーに、マップデータ(2次元リスト)を保持する辞書
    [HideInInspector]
    public Dictionary<string, List<List<int>>> _RockMapData = new Dictionary<string, List<List<int>>>();

    private void Awake()
    {
        //マップデータ読み込み処理
        _RockMapData = Road_Map_Date("Rock_MapData");
    }
